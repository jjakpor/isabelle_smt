...
(* Construction functions that perform simple normalisation*) 
...
definition re_plus::"'a::linorder regex \<Rightarrow> 'a regex" where "re_plus r \<equiv> re_concat r (re_star r)"
...
lemma re_inter_correct: "lang (re_inter r1 r2) = lang (Inter r1 r2)"
  apply(cases \<open>(r1, r2)\<close> rule: re_inter.cases)
  by auto
...
"vu (Inter r1 r2) = Inter (vu r1) (vu r2)" |
...
lemma [simp]: "c>u \<Longrightarrow> rderiv c (Range l u) = None"
  by (auto)

lemma [simp]: "c<l \<Longrightarrow> rderiv c (Range l u) = None"
  by (auto)

lemma [simp]: "l\<le>c \<and> c \<le> u \<Longrightarrow> rderiv c (Range l u) = (Const \<epsilon>)"
  by (auto)
...
next
  case (Inter r1 r2)
  then show ?case by (auto simp add: deriv_inter re_inter_correct)
next
  case (Star r)
...
definition re_contains:: "'a::linorder word \<Rightarrow> 'a regex \<Rightarrow> bool" where "re_contains w r \<equiv> nullable (rderivw w r)"
