...
abbreviation str_concat:: "uc_word \<Rightarrow> uc_word  \<Rightarrow> uc_word"  where "(str_concat) u v \<equiv> u\<cdot>v" 
abbreviation str_len:: "uc_word \<Rightarrow> int" ("\<bar>_\<bar>") where  "str_len w \<equiv> of_nat (length w)"
abbreviation str_at:: "uc_word \<Rightarrow> int \<Rightarrow> uc_word" where "str_at w i \<equiv> if i \<ge> 0 then w[(nat i); (nat i+1)] else \<epsilon>"
abbreviation str_substr:: "uc_word \<Rightarrow> int \<Rightarrow> int \<Rightarrow> uc_word"  where "str_substr w m n \<equiv> if (n \<ge> 0 \<and> 0\<le>m \<and> ((nat m) \<le> (length w)-1)) then  w[(nat m);(nat (m+n))] else \<epsilon>"
abbreviation str_prefixof:: "uc_word \<Rightarrow> uc_word \<Rightarrow> bool" where "str_prefixof \<equiv> prefix"
abbreviation str_suffixof:: "uc_word \<Rightarrow> uc_word \<Rightarrow> bool" where "str_suffixof \<equiv> suffix"
abbreviation str_contains:: "uc_word \<Rightarrow> uc_word \<Rightarrow> bool" where "str_contains \<equiv> Words.contains"
abbreviation str_indexof:: "uc_word \<Rightarrow> uc_word \<Rightarrow> int \<Rightarrow> int" 
...
abbreviation str_replace:: "uc_word \<Rightarrow> uc_word \<Rightarrow> uc_word \<Rightarrow> uc_word" where "str_replace \<equiv> replace"

abbreviation str_in_re:: "uc_word \<Rightarrow> uc_regex \<Rightarrow> bool" where "str_in_re w R \<equiv> re_contains w R"
abbreviation str_to_re:: "uc_word \<Rightarrow> uc_regex" where "str_to_re w \<equiv> regex.Const w"
...
lemma length_int_nat_sub_min: "\<bar>w\<bar> \<ge>  m \<and> n\<ge> 0 \<and> m \<ge> 0 \<Longrightarrow> int (min (nat n) ((length w)-nat m)) = (min n (\<bar>w\<bar>- m))"
  by auto
lemma of_nat_nat_inv: "x\<ge> 0 \<Longrightarrow> of_nat (nat x) = x" by auto
...
proof (unfold_locales)
  fix u v w::uc_word
  fix n m i:: int
  show "u\<cdot>(v\<cdot>w) =  (u\<cdot>v)\<cdot>w" by auto 
  show "w\<cdot>\<epsilon> = w" by auto
  show "\<epsilon>\<cdot>w = w" by auto
  show "\<bar>w\<bar> = 0 \<longleftrightarrow> w = \<epsilon>" by auto 
  show "\<bar>u\<cdot>v\<bar> = \<bar>u\<bar> + \<bar>v\<bar>" by auto
  show "str_at w n = str_substr w n 1" by (simp split: if_splits add: diff_nat_eq_if)
  show str_substr1: "0 \<le> m \<and> m < \<bar>w\<bar> \<and> 0 < n \<Longrightarrow> \<exists>!v. str_substr w m n = v \<and> (\<exists>x y. w =  x\<cdot>v\<cdot>y \<and> \<bar>x\<bar> = m \<and>  \<bar>v\<bar> = min n (\<bar>w\<bar> - m))" using substr_correct1 by auto 
  show str_substr2: "\<not>(0 \<le> m \<and> m <  \<bar>w\<bar> \<and> 0 < n) \<Longrightarrow> str_substr w m n = \<epsilon>" using substr_correct2 by presburger
  show "str_prefixof v w \<longleftrightarrow>( \<exists>u. w = v\<cdot>u)" by (simp add: prefix_def)
  show "str_suffixof v w \<longleftrightarrow> (\<exists>u. w = u\<cdot>v)" by (simp add: suffix_def)
  show "str_contains w v \<longleftrightarrow> (\<exists>x y. w = x\<cdot>v\<cdot>y)" using contains_iff_factor sublist_def by blast
  show "i\<ge> 0 \<and> str_contains (str_substr w i \<bar>w\<bar>) v \<Longrightarrow> \<exists>n. str_indexof w v i = n \<and> (\<exists>x y. w = x\<cdot>v\<cdot>y \<and> i \<le> n \<and> n = \<bar>x\<bar>) \<and> (\<forall>n'. n' < n \<longrightarrow> (\<exists>x' y'. w = x'\<cdot>v\<cdot>y' \<and> i \<le> n' \<and> n = \<bar>x'\<bar>))" using indexof_correct1 by auto
  show "i<0 \<or> \<not> str_contains (str_substr w i \<bar>w\<bar>) v  \<Longrightarrow> str_indexof w v i = -1" using indexof_correct2 by auto
  show "str_contains w v \<Longrightarrow> \<exists>x y. str_replace w v u = x\<cdot>u\<cdot>y \<and> w = x\<cdot>v\<cdot>y \<and> (\<forall> x'. \<bar>x'\<bar> < \<bar>x\<bar> \<longrightarrow> (\<nexists>y'. w=x'\<cdot>v\<cdot>y'))" using replace_first_factor by fastforce
  show "\<not> str_contains w v \<Longrightarrow> str_replace w v u = w" by (simp add: replace_id_if_not_contains)

  fix r e::uc_regex
  fix k::"uc_word set"
  fix a b::"nat"
  show "str_in_re w r \<longleftrightarrow> w \<in> (lang r)" by (auto simp add: re_contains_def derivative_correctness)
  show "lang (str_to_re w) = {w}" by simp
  show "lang re_none = {}" by simp
  show "lang re_allchar = {w. int (length w) = 1}" by simp
  show "lang (re_concat r e) = {str_concat x y |x y. x \<in> lang r \<and> y \<in> lang e}"  by (simp add: Regular.concat_def re_concat_correct)
  show "lang (Strings.re_union r e) = {w |w. w \<in> lang r \<or> w \<in> lang e}" by (simp add: Un_def re_union_correct)
  show "lang (Strings.re_inter r e) = {w |w. w \<in> lang r \<and> w \<in> lang e}"  by (auto simp add: re_inter_correct)
  show "lang (Strings.re_star r) = k \<Longrightarrow> \<epsilon> \<in> k \<and> {str_concat x y |x y. x \<in> lang r \<and> y \<in> k} \<subseteq> k" using concat_star_subset re_star_correct  Regular.concat_def by fastforce
  show "lang (Strings.re_plus r) = lang (Strings.re_concat r (Strings.re_star r))" by (simp add: re_plus_def)
  show "\<bar>u\<bar> = 1 \<and> \<bar>w\<bar> = 1 \<Longrightarrow> lang (Strings.re_range u w) = {v |v. u \<le> v \<and> v \<le> w \<and> int (length v) = 1}" using re_range_correct1 by simp
  show "\<bar>u\<bar> \<noteq> 1 \<or> \<bar>w\<bar> \<noteq> 1 \<Longrightarrow> lang (Strings.re_range u w) = {}" by (simp add: re_range_correct2)
  show "lang (re_comp r) = UNIV - lang r" by (auto simp add: re_comp_correct UC_def)
  show "lang (re_diff r e) = lang r - lang e"  by (simp add: re_diff_correct)
  show "lang (re_pow 0 r) = {\<epsilon>}" by auto
  show "\<forall>n. Strings.re_pow (Suc n) r = Strings.re_concat r (Strings.re_pow n r)"  by simp
  show "a \<le> b \<Longrightarrow> lang (Strings.re_loop a b r) = (\<Union>x\<in>{a..b}. lang (Strings.re_pow x r))" using re_loop_correct1 by presburger
  show "b < a \<Longrightarrow> lang (Strings.re_loop a b r) = {}" by (simp add: re_loop_iff2)
qed
